<?php
	...
	function connect_sql() {
		...
		// 資料庫語法
		// (table_time要補)
		$sql="SELECT phone, address, intro, day, begin, end FROM table_show
		...
		while($row = mysqli_fetch_array($result)){
			array_push($response, array("restPhone"=>$row["phone"], "restAddress"=>$row["address"], "restIntro"=>$row["intro"],
													"restDay"=>$row['day'], "restBegin" => $row['begin'], "restEnd" => $row['end'])); //
		}
